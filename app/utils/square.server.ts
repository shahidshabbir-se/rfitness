    ...
    try {
      ...
      try {
        ...
        const recentCashPayment = paymentsResponse.result.payments?.find(payment => {
          ...
          return isCashPayment && totalMoney && (totalMoney >= 2900 && totalMoney <= 3100); // £29-£31 range (in pence)
        });
...
export async function getMembersNeedingRenewal(): Promise<{
  ...
  try {
    ...
    if (!client) {
      console.warn('Square client not available for getting members needing renewal');
    ...
    for (const customer of result.customers) {
      try {
        ...
        if (subscription) {
          ...
          if (subscription.charged_through_date) {
            ...
            if (daysUntilExpiry <= 5) {
              ...
              const fullName = `${customer.givenName || ''} ${customer.familyName || ''}`.trim();
        ...
        console.error(`Error checking renewal status for customer ${customer.id}:`, error);
