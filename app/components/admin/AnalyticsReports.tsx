...
export default function AnalyticsReports({
  ...
  const filteredMembershipTypes =
    membershipTypes && membershipTypes.length > 0
      ? membershipTypes.filter((type) => type && type.type !== "Trial")
      : [];

  // Calculate max values for scaling
  const maxCheckInCount = Math.max(
    ...(checkInsByDay || []).map((day) => (day && day.count) || 0),
    1
  );
  const totalMembers = (filteredMembershipTypes || []).reduce(
    (sum, type) => sum + ((type && type.count) || 0),
    0
  );
  ...
  const handleExportReports = () => {
    ...
    const exportData = {
      ...
      peakHours,
      topMembers,
      averageDailyCheckIns: Math.round(
        checkInsByDay.reduce((sum, day) => sum + day.count, 0) /
          checkInsByDay.length
      ),
            ...
            {checkInsByDay.map((day, index) => (
              <div key={index} className="flex flex-1 flex-col items-center">
                <div
                  ...
                  style={{
                    height: `${(day.count / maxCheckInCount) * 100}px`,
                    minHeight: "4px",
                  }}
                ></div>
          ...
          <div className="space-y-3">
            {peakHours.map((hour, index) => {
              ...
              const percentage =
                hour.percentage ??
                Math.round(
                  (hour.count / Math.max(...peakHours.map((h) => h.count))) *
                    100
                );
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4">
      ...
      <div className="mt-2 flex items-baseline">
        ...
        <p
          className={`ml-2 text-sm font-medium ${isPositive ? "text-green-600" : "text-red-600"}`}
        >
          {isPositive ? "↑" : "↓"} {change}
        </p>
      </div>
