...
export default function CheckInNotifications({ notifications }: CheckInNotificationsProps) {
          ...
          {notifications.map((notification) => (
            ...
            >
              ...
              <div className="ml-4 flex-1">
                <div className="flex items-center justify-between">
                  <p className="font-medium text-gray-900">{notification.customerName}</p>
                  <span className="text-sm text-gray-500">
                    {new Date(notification.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
                <p className={`text-sm ${notification.success ? 'text-green-700' : 'text-red-700'}`}>
